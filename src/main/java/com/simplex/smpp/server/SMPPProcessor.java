...
import com.simplex.smpp.toolpooler.ClientBalancePooler;
import com.simplex.smpp.toolpooler.ClientPropertyPooler;
...
import com.simplex.smpp.toolpooler.RouteSMSPooler;
import com.simplex.smpp.toolpooler.SMSTransactionOperationPooler;
import com.simplex.smpp.toolpooler.SenderIdSMSPooler;
import com.simplex.smpp.toolpooler.TelecomPrefixPooler;
...
//					theCharSet = CharsetUtil.CHARSET_GSM;
	...
	private void saveInitialData(
			String messageId, LocalDateTime receiverDateTime, String receiverData, String receiverClientResponse,
            ...
            if (!future.await()) {
               ...
               LoggingPooler.doLog(logger, "INFO", "SMPPProcessor", "sendRequestPdu", false, false,
					   true, messageId, "deliver_sm_resp: commandStatus [" + deliverSmResp.getCommandStatus() + "=" +
							   deliverSmResp.getResultMessage() + "]", null);
            } else {
	...
	private void doProcessTheSMS() {
		...
		String errorCode; // DEFAULT errCode
        byte deliveryState;
		byte esmeErrCode;
		...
		try {
					...
					if(isRouteDefined(this.clientId, this.clientSenderId, this.systemId, telecomId)){
						...
						if(businessModel.equals("PREPAID")){
							...
							LoggingPooler.doLog(logger, "DEBUG", "SMPPProcessor",
									"doProcessTheSMS", false, false, true, messageId, "Session ID: " + sessionId +
											". Checking initial balance - divisionBalance: " + divisionBalance, null);
						} else {
						...
						if (isBalanceEnough) {
    						...
    						LoggingPooler.doLog(logger, "INFO", "SMPPProcessor", "doProcessTheSMS",
									false, false, true, messageId, "Session ID: " + sessionId +
											...
											", esmeErrCode: SUBMITE FAILED.", null);
						}
					} else {
						...
						LoggingPooler.doLog(logger, "INFO", "SMPPProcessor", "doProcessTheSMS",
								false, false, true, messageId, "Session ID: " + sessionId +
										...
										", esmeErrCode: SUBMIT FAILED.", null);
					}
				} else {
					// Prefix is VALID
					// Set MSISDN property
					...
					if(isRouteDefined(this.clientId, this.clientSenderId, this.systemId, telecomId)){
						...
						LoggingPooler.doLog(logger, "DEBUG", "SMPPProcessor", "doProcessTheSMS",
								false, false, true, messageId, "MULTI-COUNTRY CASE - Session ID: " + sessionId +
										". Business Mode: " + businessModel, null);
						...
						LoggingPooler.doLog(logger, "INFO", "SMPPProcessor", "doProcessTheSMS",
								false, false, true, messageId, "Session ID: " + sessionId +
										...
										", esmeErrCode: INVALID NUMBER DESTINATION.", null);
					}
				}
			} else {
				if (isSenderIdValid(this.clientId, "ALL")) {
					// clientSenderId is valid
					clientSenderIdId = clientSenderId.trim() + "-" + this.clientId.trim();
					...
					LoggingPooler.doLog(logger, "INFO", "SMPPProcessor", "doProcessTheSMS", false,
							false, true, messageId, "Session ID: " + sessionId + ". SENDERID IS NOT DEFINED. errCode: " +
									errorCode + ", esmeErrCode: INVALID SOURCE ADDRESS.", null);
				}
			}
			...
			saveInitialData(this.messageId, incomingDateTime, receiverData,
					receiverResponse, incomingDateTime, msisdn, shortMessage, countryCode,
					prefix, telecomId, errorCode, receiverType, clientSenderIdId, clientSenderId,
					clientPricePerSubmit, clientCurrency, messageEncoding, messageLength, smsCount);
			...
			if (!errorCode.trim().startsWith("00")) {
				...
				LoggingPooler.doLog(logger, "DEBUG", "SMPPProcessor", "doProcessTheSMS - " +
						this.sessionId, false, false, false, messageId, "Sending DLR with session: " +
				...
				LoggingPooler.doLog(logger, "DEBUG", "SMPPProcessor", "doProcessTheSMS - " +
						this.sessionId, false, false, false, messageId, "jsonIncoming: " + jsonIncoming +
