...
//									break;
			...
			LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "getCharsetByDataCoding - " + this.sessionId, false, false, false, "",
					"dataCoding: " + dataCoding + " -> charset: " + theCharSet, null);				
		} catch (Exception e) {
			e.printStackTrace();
			
			LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "getCharsetByDataCoding - " + this.sessionId, true, false, false, "",
    ...
    public PduResponse firePduRequestReceived(PduRequest pduRequest) {
        ...
        if(pduRequest.getCommandId() == SmppConstants.CMD_ID_ENQUIRE_LINK){
			...
			System.out.println("Enquire Link Request from " + Objects.requireNonNull(session).getConfiguration().getSystemId() + " - " +
			...
			LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "firePduRequestReceived", false, false, true, "", 
					this.sessionId + " - " + Objects.requireNonNull(session).getConfiguration().getSystemId() + " - PDU Request: CMD_ID_BIND_TRANSCEIVER. Responded!", null);
			
		} else if (pduRequest.getCommandId() == SmppConstants.CMD_ID_SUBMIT_SM) {
			try {
				LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "firePduRequestReceived", false, false, true, "", 
						this.sessionId + " - " + Objects.requireNonNull(session).getConfiguration().getSystemId() + " - PDU Request: CMD_ID_SUBMIT_SMS.", null);
                ...
                LoggingPooler.doLog(logger, "INFO", "SimpleSMPPSessionHandler", "firePduRequestReceived", false, false, true, "", 
						this.sessionId + " - " + session.getConfiguration().getSystemId() + " - Assigning messageId: " + messageId, null);
				...
				LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "firePduRequestReceived", false, false, true, "", 
						this.sessionId + " - " + session.getConfiguration().getSystemId() + " - isUDH (multipart sms): " + isUdh, null);
				...
				if (isUdh) {
					...
					LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "firePduRequestReceived", false, false, true, "",
							this.sessionId + " - " + session.getConfiguration().getSystemId() + " - theSMSPart: " + theSMSPart+ " - theSMSPart length: " + theSMSPart.length(), null);
					...
					if (maxMessageCount == currentMessageCount && allSMSMessage.length != 0) {
						...
						LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "firePduRequestReceived", false, false, true, messageId,
						...
						Thread incomingTrxProcessor = new Thread(new SMPPProcessor(messageId, systemId, remoteIpAddress, allMessageIds, clientSenderId, msisdn, allSMSMessage,
	                ...
	                if (shortMessage.length == 0) {
	                	LoggingPooler.doLog(logger, "INFO", "SimpleSMPPSessionHandler", "firePduRequestReceived", false, false, true, messageId, 
	                ...
	                // Run thread smppIncomingTrxProcessor
	                Thread incomingTrxProcessor = new Thread(new SMPPProcessor(messageId, systemId, remoteIpAddress, "", clientSenderId, msisdn, shortMessage,
	                		dataCoding, clientId, mtSourceAddress, mtDestinationAddress, clientPropertyPooler, clientBalancePooler, smsTransactionOperationPooler, rabbitMqPooler,
	                		rabbitMqConnection, redisPooler, sessionRef, logger));
	                incomingTrxProcessor.start();
				...
				LoggingPooler.doLog(logger, "DEBUG", "SimpleSMPPSessionHandler", "firePduRequestReceived", true, false, true, "", 
