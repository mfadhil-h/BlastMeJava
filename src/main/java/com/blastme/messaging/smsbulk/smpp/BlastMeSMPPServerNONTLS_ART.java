...
public class BlastMeSMPPServerNONTLS_ART {
	...
	private static String SMSQueueName = "SMPP_INCOMING_ART";
	
	public BlastMeSMPPServerNONTLS_ART() {
		// Set timezone
...
//	        configuration.setUseSsl(true);
	...
	public static void main(String[] args) {
		BlastMeSMPPServerNONTLS_ART blastmeSmppServer = new BlastMeSMPPServerNONTLS_ART();
	...
	public static class DefaultSmppServerHandler implements SmppServerHandler {
        ...
        public DefaultSmppServerHandler() {
        	try {
        		...
        		execClientDLR.schedule(new ClientDLRSubmitter(), 10, TimeUnit.SECONDS);
        ...
        class ClientDLRSubmitter implements Runnable {
			...
			public void run() {
				// TODO:
				// BUAT THREAD TERPISAH UTK CHECK STATUS READ QUEUE HARUS TETAP ON.
				
				try {
					System.out.println("Starting DLR Submitter ...");
					LoggingPooler.doLog(logger, "INFO", "BlastMeSMPPServer - ClientDLRSubmmiter", "ClientDLRSubmmiter", false, false, false, "", 
							"Starting DLR Submitter. xIx", null);
					readDLRQueue();				
				} catch (Exception e) {
...
//										break;
        ...
        public PduResponse firePduRequestReceived(PduRequest pduRequest) {
            ...
            if(pduRequest.getCommandId() == SmppConstants.CMD_ID_ENQUIRE_LINK){
				...
				try {
					...
					System.out.println("isUDH: " + isUdh);
					...
					if (isUdh) {
		                ...
		                if (shortMessage.length == 0) {
			                ...
			                System.out.println("tlvContent: " + strTlvContent);
        ...
        class SMPPIncomingTrxProcessor implements Runnable {
    		...
    		public SMPPIncomingTrxProcessor(String theMessageId, String theSystemId, String theRemoteIpAddress, String theClientSenderId, String theMsisdn, 
    			...
    			if (this.clientId.equals("TAX20092808916")) {
    				this.shortMessage = CharsetUtil.decode(bShortMessage, CharsetUtil.CHARSET_UTF_8);
    				LoggingPooler.doLog(logger, "DEBUG", "EliandrieSMPPServerNeo - SMPPIncomingTrxProcessor", "SMPPIncomingTrxProcessor - " + this.sessionId, false, false, false, messageId, 
        					"Encoded with UTF-8: " + this.shortMessage, null);	
    				
//    				this.shortMessage = CharsetUtil.decode(bShortMessage, CharsetUtil.CHARSET_UCS_2);
//    				LoggingPooler.doLog(logger, "DEBUG", "EliandrieSMPPServerNeo - SMPPIncomingTrxProcessor", "SMPPIncomingTrxProcessor - " + this.sessionId, false, false, false, messageId, 
//        					"Encoded with UCS2: " + this.shortMessage, null);	
    					...
    					if (jsonMsisdn.getBoolean("isValid") == true) {
							...
							if(isRouteDefined(this.clientId, this.clientSenderId, this.systemId, telecomId) == true){
								...
								if (isBalanceEnough == true) {
									...
									// For alienics, make it as delivered
//									if (this.clientId.equals("AMS20090911959")) {
//										errorCode = "000";
...
//			    								"Client Alienics -> errCode: 000,  DELIVERED", null);
//									}
