...
public class TransceiverTelinVoiceBlastWithThrottling {
	...
	public TransceiverTelinVoiceBlastWithThrottling(String queueName, int theTPS) {
		// Set timezone
	...
	public static void main(String[] args) {
		...
		TransceiverTelinVoiceBlastWithThrottling cpass = new TransceiverTelinVoiceBlastWithThrottling(queueTelin, theTPS);
		
		System.out.println("Starting Transceiver TELIN for VOICE BLAST by reading queue " + queueTelin);
		cpass.readQueue();		
		System.out.println("Transceiver TELIN for VOICE BLAST is RUNNING with queue " + queueTelin);
	}
	...
	private class hitTelin implements Runnable {
		...
		private String getTelinURL(String messageId, String accId, String tokenId, String apiUserName){
			...
			try{
				...
				if (accId.trim().length() > 0 || tokenId.trim().length() > 0) {
					theURL = "https://" + accId + ":" + tokenId + "@jatis.neuapix.com/restcomm/2012-04-24/Accounts/" + accId + "/Calls";
				} 				
