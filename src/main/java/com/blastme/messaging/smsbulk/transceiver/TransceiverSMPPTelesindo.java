...
public class TransceiverSMPPTelesindo {
    private static Logger logger;
    
    private static String host = "122.102.41.66";
    private static int port = 7071;
    private static String sysId = "MODNT";
    private static String pass = "w0pj931p";
    
//    private static String host = "localhost";
//    private static int port = 2776;
//    private static String sysId = "raya01";
//    private static String pass = "ray01";

    private static String queueName = "TELESINDO01";
...
//    private static int redisExpiry = 7 * 24 * 60 * 60; // 7 days from last operation
	...
	private static int sessionCount = 3; // FOR NOW JUST FOR 1 SESSION ONLY. MULTI SESSION IS STILL BUGGY!
	...
	public TransceiverSMPPTelesindo() {
		new LoggingPooler();
		
		try {
			// Setup logger
			logger = LogManager.getLogger("TRANSCEIVER_SMPP_TELESINDO");

			// Initiate RedisPooler
//			redisPooler = new RedisPooler();
//			redisCommand = redisPooler.redisInitiateConnection();

			LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTelesindo", "TransceiverSMPPTelesindo", false, false, false, "", "Module TransceiverSMPPTelesindo is running.", null);				
		} catch (Exception e) {
			e.printStackTrace();
			LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTelesindo", "TransceiverSMPPTelesindo", true, false, false, "", 
					"Failed to run module TransceiverSMPPTelesindo. Error occured.", e);	
	...
	private void startTheEngine() {
		try {
            ...
            for (int x = 0; x < sessionCount; x++) {
            	...
            	executorX.execute(new Transmitter(queueName, clientBootStrapArr[x], sessionHandlerArr[x], config0, "SMPPTELESINDO-0" + x));
            }
...
//            DefaultSmppSessionHandler sessionHandler = new ClientSmppSessionHandler();
	...
	public static void main(String[] args) {
		TransceiverSMPPTelesindo trcSMPP = new TransceiverSMPPTelesindo();
		trcSMPP.startTheEngine();
	}
    ...
    public static class Transmitter implements Runnable{
    	...
    	public Transmitter(String theQueueName, DefaultSmppClient clientBootstrap, DefaultSmppSessionHandler sessionHandler, SmppSessionConfiguration config0, String sessionName) {
    		...
    		try {
				...
				LoggingPooler.doLog(logger, "DEBUG", "TransceiverSMPPTELESINDO", "Transmitter", false, false, false, "", 
						"Transmitter - TransceiverSMPPTELESINDO is READY.", null);				
        		
    		} catch (Exception e) {
    			e.printStackTrace();
				LoggingPooler.doLog(logger, "DEBUG", "TransceiverSMPPTELESINDO", "Transmitter", true, false, false, "", 
						"Transmitter - TransceiverSMPPTELESINDO is FAILED to INITIATE.", e);				
    		}
		}
		...
		private void readQueue() {
			try{
				...
				LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTELESINDO", "readQueue - " + queueName, false, false, true, "", 
...
//							thread.start();
							...
							// Bit slow but safer
							SMPPSubmitter theHitter = new SMPPSubmitter(theSession, message);
							boolean hitStatus = theHitter.Transmit(theSession, message);
							
							if (hitStatus == false) {
								// Reload channel - rebind
								if (!(theSession.isBinding() == true || theSession.isBound() == true)) {
									// - NOT BOUND, rebinding
									LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTELESINDO", "readQueue - " + queueName, false, false, true, "", 
											"Session is CLOSED, do re-binding now!", null);
									theSession = Bind();
								}
							}
						} finally {
				...
				e.printStackTrace();
				LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTELESINDO", "readQueue - " + queueName, true, false, false, "", 
		...
		public void run() {
			try {
				//SmppSession theSession = Bind();
				theSession = Bind();
				
				if (theSession != null) {
					theSession.getConfiguration().setName(this.sessionName);
					LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTELESINDO", "run", false, false, true, "", 
							"Binding session: " + this.sessionName, null);

					readQueue();
				} else {
					LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTELESINDO", "readQueue - " + queueName, false, false, false, "", 
		...
		private class SMPPSubmitter implements Runnable {
			...
			public boolean Transmit(SmppSession theSession, String queueMessage){
...
//				jsonQueuedMessage.put("routedVendorId", routedVendorId);		
				...
				try{				
					...
					LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTELESINDO", "Transmitting - SESSION NAME: " + theSession.getConfiguration().getName(), false, false, true, theMessageId, 
							"Start Transmitting!", null);
					
		            byte[] textBytes = CharsetUtil.encode(theMessage, CharsetUtil.CHARSET_GSM);
					...
					LoggingPooler.doLog(logger, "INFO", "TransceiverSMPPTELESINDO", "Transmitting: " + theSession.getConfiguration().getName(), false, false, true, theMessageId, 
		        	...
		        	ex.printStackTrace();
					LoggingPooler.doLog(logger, "DEBUG", "TransceiverSMPPTELESINDO", "Transmitter: " + theSession.getConfiguration().getName(), true, false, false, "", 
							"Failed to send message. TIME OUT ERROR OCCURED.", ex);	
					
					isSuccessSubmit = false;
		        } catch (com.cloudhopper.smpp.type.SmppChannelException ey) {
		        	ey.printStackTrace();
					LoggingPooler.doLog(logger, "DEBUG", "TransceiverSMPPTELESINDO", "Transmitter: " + theSession.getConfiguration().getName(), true, false, false, "", 
							"Failed to send message. CHANNEL IS CLOSED.", ey);	
