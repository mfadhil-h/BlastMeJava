...
public class ContextualSenderNew implements Job{
	private static String queueIncomingMessage = "VOICE_INCOMING";
	private static String queueRinglerIncoming = "RINGLERR_INCOMING";
	private static String smsChannel = "CONTEXTUAL_WEBUPLOADER";
	...
	public ContextualSenderNew() {
		// Alert of initing
		System.out.println("INTIATING CONTEXTUAL SENDER!!!");
	}
	...
	private void doSending( ) {
		try {
			System.out.println("Checking message to send VOICE CONTEXTUAL using query: " + statementGetMesssageToSend.toString());
			...
			while (resultSet.next()) {
				...
				// For sending ringlerr, messageId needs to change messageId_ringlerr, clientSenderId and clientSenderIdId started with ringlerr_
				doSendingRinglerr(messageId + "_ringlerr", batchId, msisdn,ringlerrMessage, sendingSchedule, "ringlerr_" + clientSenderIdId, "ringlerr_" + clientSenderId, username, clientId);
				doSendingVoice(messageId, batchId, msisdn, message, sendingSchedule, clientSenderIdId, clientSenderId, username, clientId);
			}
	...
	private void doSendingVoice(String messageId, String batchId, String msisdn, String message, LocalDateTime sendingSchedule, String clientSenderIdId,
			String clientSenderId, String username, String clientId) {
		try {
			System.out.println("Checking message to send VOICE CONTEXTUAL using query: " + statementGetMesssageToSend.toString());
				...
				if (LocalDateTime.now().isAfter(sendingSchedule)) {
					...
					if(RouteSMSPooler.jsonRouteSMSProperty.has(routeId)){
						...
						if (trxStatusCurrent.trim().length() == 0 || trxStatusCurrent.trim().equals(errorCode)) {							
							try {
								smsTransactionOperationPooler.saveInitialSMPPData(messageId, now, batchId, "VOICE_BROADCAST", "", 
										...
										username, clientPricePerSubmit, clientCurrency, "VOICE", messageLength, messageCount);
							} catch (Exception ex) {
								ex.printStackTrace();
						...
						smsTransactionOperationPooler.saveInitialSMPPData(messageId, now, batchId, "VOICE_BROADCAST", "", 
								"", now, now, msisdn, message, countryCode, prefix, telecomId, errorCode, 
								"AUTOGENAUPLOADER", clientSenderIdId, clientSenderId, clientId, 
								username, 0.00, "", "VOICE", messageLength, messageCount);
					}					
				} else {
...
//			resultSet.close();
	...
	private void doSendingRinglerr(String messageId, String batchId, String msisdn, String message, LocalDateTime sendingSchedule, String clientSenderIdId,
			String clientSenderId, String username, String clientId) {
		try {
			System.out.println("Checking message to send RINGLERR using query: " + statementGetMesssageToSend.toString());
...
//			int count = 0;
//			while (resultSet.next()) {
//				String messageId = resultSet.getString("message_id") + "_ringlerr";
//				String batchId = resultSet.getString("batch_id");
//				String msisdn = resultSet.getString("msisdn");
//				String message = resultSet.getString("message");
//				String ringlerrMessage = resultSet.getString("ringlerr_message");
...
//	    		LoggingPooler.doLog(logger, "DEBUG", "VoiceBlastSender", "doSending", false, false, false, "", 
//	    				count + ". Sending batchId: " + batchId + ", clientId: " + clientId + ", userName: " + username + 
//	    				" -> msisdn: " + msisdn + ", clientSenderIdId: " + clientSenderIdId + ", ringlerrMessage: " + ringlerrMessage, null);
				...
				if (LocalDateTime.now().isAfter(sendingSchedule)) {
					...
					// Get clientPrice and clientCurrency
					// For ringlerr, all the cleintSenderId will be started by "ringlerr-"
					String routeId =  clientSenderIdId + "-" + username + "-" + telecomId;
					...
					if(RouteSMSPooler.jsonRouteSMSProperty.has(routeId)){
						...
						if (trxStatusCurrent.trim().length() == 0 || trxStatusCurrent.trim().equals(errorCode)) {							
							...
							// Publish jsonIncoming to queue INCOMING_SMS
							channelRabbitPublish.queueDeclare(queueRinglerIncoming, true, false, false, null);
							channelRabbitPublish.basicPublish("", queueRinglerIncoming, MessageProperties.PERSISTENT_TEXT_PLAIN, jsonIncoming.toString().getBytes());
							LoggingPooler.doLog(logger, "DEBUG", "VoiceBlastSender", "doSending", false, false, false, messageId, 
									"Sending to queue " + queueRinglerIncoming +". jsonIncoming: " + jsonIncoming.toString() + " published SUCCESSfully!", null);
						} else {
							System.out.println("Skip sending message due to being processed in engine.");
						}
