...
public class PersonalAutogenProcessor {
	private static Logger logger;	
	private Tool tool;
	
	private static String queueBatchToProcess = "PERSONAL_AUTOGEN_SMS_TOPUSH_BATCHID";
...
//	private static RedisCommands<String, String> redisCommand;
	...
	private PreparedStatement statementPersonalAutogenDetail = null;
	...
	public PersonalAutogenProcessor() {
		// Set timezone
		...
		// Setup logger
		logger = LogManager.getLogger("PERSONAL_AUTOGEN_PROCESSOR");
...
//    		redisCommand = redisPooler.redisInitiateConnection();
    		...
    		// Select job
			statement = connection.prepareStatement("select batch_id, user_id, message, file_path, trx_datetime, is_processed from personal_autogen_sms_uploader where batch_id = ?");

    		// Get autogen_a_number as batchId
			statementIsProcessed = connection
					.prepareStatement("update personal_autogen_sms_uploader set is_processed = true where batch_id = ?");
			
			// Insert into voice detail
			statementPersonalAutogenDetail = connection
					.prepareStatement("INSERT INTO personal_autogen_sms_upload_detail(message_id, batch_id, msisdn, message, "
	...
	private void insertVoiceUploadDetail(String messageId, String batchId, String bNumber, String message, boolean isApproved, String userName) {
		try {
			...
			statementPersonalAutogenDetail.setString(1, messageId);
			statementPersonalAutogenDetail.setString(2, batchId);
			statementPersonalAutogenDetail.setString(3, bNumber);
			statementPersonalAutogenDetail.setString(4, message);
			statementPersonalAutogenDetail.setObject(5, LocalDateTime.now());
			statementPersonalAutogenDetail.setString(6, userName);
			
			System.out.println("Query: " + statementPersonalAutogenDetail.toString());
			
			int x = statementPersonalAutogenDetail.executeUpdate();
	...
	private void readQueue(){
		...
		try{
			...
			LoggingPooler.doLog(logger, "INFO", "PersonalAutogenProcessor", "readQueue", false, false, true, "", 
			...
			LoggingPooler.doLog(logger, "INFO", "PersonalAutogenProcessor", "readQueue", true, false, false, "", 
	...
	public static void main(String[] args) {
		...
		System.out.println("Starting Personal Autogen A# Processor ...");
		
		PersonalAutogenProcessor processor = new PersonalAutogenProcessor();
		processor.readQueue();
	}
