...
public class ContextualProcessorTelin {
	private static Logger logger;	
	private Tool tool;
	
	private static String queueBatchToProcess = "CONTEXTUAL_BLAST_TOPUSH_BATCHID_TELIN";
...
//	private static RedisCommands<String, String> redisCommand;
	...
	private PreparedStatement statementContextualDetail = null;
	...
	public ContextualProcessorTelin() {
		// Set timezone
		...
		// Setup logger
		logger = LogManager.getLogger("CONTEXTUAL_PROCESSOR_TELIN");
		...
		//batchFileDirectory = "/app/web_indosat/uploadedBNumberMsgParameter/";
		//batchFileDirectory = "/app/web_atlasat/uploadedBNumberMsgParameter/";
		batchFileDirectory = "/app/web_telin/uploadedBNumberMsgParameter/";
		//batchFileDirectory = "D:\\ChandraDocument\\Projects\\KolangKalingWebClient\\uploadedBNumberMsgParameter\\";
...
//    		redisCommand = redisPooler.redisInitiateConnection();
    		...
    		LoggingPooler.doLog(logger, "INFO", "ContextualProcessor", "ContextualProcessor", false, false, false, "", 
    				"Application VoiceBlastProcessor is loaded and run.", null);
    		
    		// Select job
			statement = connection.prepareStatement("select batch_id, user_id, message, ringlerr_message, file_path, trx_datetime, is_processed from contextual_blast_uploader where batch_id = ?");

    		// Get autogen_a_number as batchId
			statementIsProcessed = connection
					.prepareStatement("update contextual_blast_uploader set is_processed = true where batch_id = ?");
			
			// Insert into voice detail
//			statementContextualDetail = connection
//					.prepareStatement("INSERT INTO contextual_blast_upload_detail(message_id, batch_id, msisdn, message, ringlerr_message, "
//							+ "processed_datetime, processed_username) VALUES (?, ?, ?, ?, ?, ?, ?)");
			// Temporary
			statementContextualDetail = connection
					.prepareStatement("INSERT INTO contextual_blast_upload_detail(message_id, batch_id, msisdn, message, ringlerr_message, "
							+ "processed_datetime, processed_username, is_review_approved) VALUES (?, ?, ?, ?, ?, ?, ?, true)");

        } catch(Exception e) {
        	e.printStackTrace();
    		LoggingPooler.doLog(logger, "INFO", "ContextualProcessor", "ContextualProcessor", true, false, false, "", 
	...
	private void updateIsProcessed(String batchId){
		try{			
			...
			LoggingPooler.doLog(logger, "DEBUG", "ContextualProcessor", "updateIsProcessed", false, false, false,"", 
					"Succesfully update batchId " + batchId + " ALREADY PROCESSED TO TRUE.", null);	
		} catch (Exception e) {
			e.printStackTrace();
			
			LoggingPooler.doLog(logger, "DEBUG", "ContextualProcessor", "updateIsProcessed", true, false, false,"", 
					"Failed to update batchId " + batchId + " ALREADY PROCESSED TO TRUE.", e);				
		}
	}
	
	private void insertContextualUploadDetail(String messageId, String batchId, String bNumber, String message, String ringlerrMessage, boolean isApproved, String userName) {
		try {
			
			statementContextualDetail.setString(1, messageId);
			statementContextualDetail.setString(2, batchId);
			statementContextualDetail.setString(3, bNumber);
			statementContextualDetail.setString(4, message);
			statementContextualDetail.setString(5, ringlerrMessage);
			statementContextualDetail.setObject(6, LocalDateTime.now());
			statementContextualDetail.setString(7, userName);
			
			System.out.println("Query: " + statementContextualDetail.toString());
			
			int x = statementContextualDetail.executeUpdate();
			
			if (x > 0) {
				System.out.println("Successfully insert detail contextual upload - batchId: " + batchId + ", bNumber: " + bNumber);
			} else {
				System.out.println("Failed to insert detail contextual upload - batchId: " + batchId + ", bNumber: " + bNumber);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void doMergingProcess( String batchId, String fileName, String theMessageTemplate, String ringlerrMessage, String userName) {
		...
		try {
			...
			LoggingPooler.doLog(logger, "DEBUG", "ContextualBlastProcessor", "doMergingProcess", false, false, false,"", 
			...
			for (int x = 1; x <= rowCount; x++) {
				...
				insertContextualUploadDetail(messageId, batchId, bNumber, messagePenampung, ringlerrMessage, false, userName);
			}
			...
			LoggingPooler.doLog(logger, "DEBUG", "ContextualBlastProcessor", "doMerghingProcess", true, false, true,"", 
	...
	private void processQueueBatch(String queueMessage){
		try{			
			...
			LoggingPooler.doLog(logger, "DEBUG", "ContextualBlastProcessor", "processQueueBatch", false, false, false,"", 
			...
			try{
				...
				while(resultSet.next()){	
					...
					String ringlerrMessage = resultSet.getString("ringlerr_message");
					
					LoggingPooler.doLog(logger, "DEBUG", "VoiceBlastProcessor", "processQueueBatch", false, false, false,"", 
							"batchId: " + batchId + ", fileName: " + fileName + ", theMessageTemplate: " + theMessage + 
							", ringlerrMessage: " + ringlerrMessage, null);	

					// Process the merging between excel file and the message
					doMergingProcess(batchId, fileName, theMessage, ringlerrMessage, username);
				...
				e.printStackTrace();
	    		LoggingPooler.doLog(logger, "INFO", "ContextualProcessor", "processQueueBatch", true, false, false, "", 
	...
	private void readQueue(){
		...
		try{
			...
			LoggingPooler.doLog(logger, "INFO", "ContextualProcessor", "readQueue", false, false, true, "", 
			...
			LoggingPooler.doLog(logger, "INFO", "ContextualProcessor", "readQueue", true, false, false, "", 
	...
	public static void main(String[] args) {
		...
		System.out.println("Starting Contextual Blast Processor ...");
		
		ContextualProcessorTelin processor = new ContextualProcessorTelin();
		processor.readQueue();
	}
