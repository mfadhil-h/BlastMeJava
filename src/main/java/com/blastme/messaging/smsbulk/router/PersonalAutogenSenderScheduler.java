...
public class PersonalAutogenSenderScheduler {
	...
	public PersonalAutogenSenderScheduler() {
		// Set timezone
        ...
        try {
    		...
    		statementGetMesssageToSend = connection
    				.prepareStatement("select dtl.message_id, dtl.batch_id, dtl.msisdn, dtl.message, \n" + 
    						"mmn.sending_schedule, dtl.processed_username, cuw.client_id from personal_autogen_sms_upload_detail as dtl \n" + 
    						"left join personal_autogen_sms_uploader as mmn on dtl.batch_id = mmn.batch_id \n" + 
...
//							+ "where (vdet.is_sent IS NULL or vdet.is_sent = false) and vdet.batch_id = ?");
			...
			statementUpdateDetail = connection
					.prepareStatement("update personal_autogen_sms_upload_detail set is_sent = true, sent_datetime = ?, "
	...
	public void runTheSendingJob() {
		try {
			...
			JobDetail job1 = JobBuilder.newJob(PersonalAutogenSender.class)
	...
	public static void main(String[] args) {
		PersonalAutogenSenderScheduler scheduler = new PersonalAutogenSenderScheduler();
