...
public class ContextualSenderScheduler {
	...
	public ContextualSenderScheduler() {
		// Set timezone
		...
		// Setup logger
		logger = LogManager.getLogger("CONTEXTUAL_SENDER_SCHEDULER");
        ...
        try {
    		...
    		LoggingPooler.doLog(logger, "INFO", "ContextualBlastSender", "ContextualBlastSender", false, false, false, "", 
    				"Application VoiceBlastProcessor is loaded and run.", null);
    		
    		// Select message to send
    		statementGetMesssageToSend = connection
    				.prepareStatement("select dtl.message_id, dtl.batch_id, dtl.msisdn, dtl.message, dtl.ringlerr_message, mmn.client_sender_id_id, csd.sender_id, "
    						+ "mmn.sending_schedule, dtl.processed_username, cuw.client_id from contextual_blast_upload_detail as dtl "
    						+ "left join contextual_blast_uploader as mmn on dtl.batch_id = mmn.batch_id "
			...
			statementReadDetail = connection
					.prepareStatement("select vdet.msisdn, vdet.message, vdet.ringlerr_message, vup.client_sender_id_id, csi.sender_id from "
							+ "contextual_blast_upload_detail as vdet left join contextual_blast_uploader as vup on "
			...
			statementUpdateDetail = connection
					.prepareStatement("update contextual_blast_upload_detail set is_sent = true, sent_datetime = ?, "
							+ "sent_status = ?, route_key = ? where batch_id = ? and msisdn = ?");
			
        } catch(Exception e) {
        	e.printStackTrace();
    		LoggingPooler.doLog(logger, "INFO", "ContextualBlastSender", "ContextualBlastSender", true, false, false, "", 
	...
	public void runTheSendingJob() {
		try {
			...
			JobDetail job1 = JobBuilder.newJob(ContextualSenderNew.class)
	...
	public static void main(String[] args) {
		ContextualSenderScheduler scheduler = new ContextualSenderScheduler();
