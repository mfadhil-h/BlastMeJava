...
public class WABlastProcessor {
	private static Logger logger;	
	private Tool tool;
	
	private static String queueBatchToProcess = "WA_BLAST_TOPUSH_BATCHID";
...
//	private static RedisCommands<String, String> redisCommand;
	...
	public WABlastProcessor() {
		...
		// Setup logger
		logger = LogManager.getLogger("WABLAST_PROCESSOR");
		...
		batchFileDirectory = "/app/web/uploadedBNumberMsgParameter/";
...
//    		redisCommand = redisPooler.redisInitiateConnection();
    		...
    		LoggingPooler.doLog(logger, "INFO", "WABlastProcessor", "WABlastProcessor", false, false, false, "", 
    				"Application VoiceBlastProcessor is loaded and run.", null);
    		
    		// Select job
			statement = connection.prepareStatement("select batch_id, user_id, message, file_path, trx_datetime, is_processed from wa_blast_uploader where batch_id = ?");

    		// Get autogen_a_number as batchId
			statementIsProcessed = connection
					.prepareStatement("update wa_blast_uploader set is_processed = true where batch_id = ?");
			
			// Insert into voice detail
			statementVoiceDetail = connection
					.prepareStatement("INSERT INTO wa_blast_upload_detail(message_id, batch_id, msisdn, message, "
							+ "processed_datetime, processed_username) VALUES (?, ?, ?, ?, ?, ?)");

        } catch(Exception e) {
        	e.printStackTrace();
    		LoggingPooler.doLog(logger, "INFO", "WABlastProcessor", "WABlastProcessor", true, false, false, "", 
	...
	private void updateIsProcessed(String batchId){
		try{			
			...
			LoggingPooler.doLog(logger, "DEBUG", "WABlastProcessor", "updateIsProcessed", false, false, false,"", 
					"Succesfully update batchId " + batchId + " ALREADY PROCESSED TO TRUE.", null);	
		} catch (Exception e) {
			e.printStackTrace();
			
			LoggingPooler.doLog(logger, "DEBUG", "WABlastProcessor", "updateIsProcessed", true, false, false,"", 
	...
	private void doMergingProcess( String batchId, String fileName, String theMessageTemplate, String userName) {
		...
		try {
			...
			LoggingPooler.doLog(logger, "DEBUG", "WABlastProcessor", "doMergingProcess", false, false, false,"", 
			...
			LoggingPooler.doLog(logger, "DEBUG", "WABlastProcessor", "doMergingProcess", true, false, true,"", 
	...
	private void processQueueBatch(String queueMessage){
		try{			
			...
			LoggingPooler.doLog(logger, "DEBUG", "WABlastProcessor", "processQueueBatch", false, false, false,"", 
	...
	private void readQueue(){
		...
		try{
			...
			LoggingPooler.doLog(logger, "INFO", "WABlastProcessor", "readQueue", false, false, true, "", 
			...
			LoggingPooler.doLog(logger, "INFO", "WABlastProcessor", "readQueue", true, false, false, "", 
	...
	public static void main(String[] args) {
		...
		System.out.println("Starting WA Blast Processor ...");
		
		WABlastProcessor processor = new WABlastProcessor();
		processor.readQueue();
	}
