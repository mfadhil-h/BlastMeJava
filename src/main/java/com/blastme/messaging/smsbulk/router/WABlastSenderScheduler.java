...
public class WABlastSenderScheduler {
	...
	public WABlastSenderScheduler() {
		...
		// Setup logger
		logger = LogManager.getLogger("WABLAST_SENDER");
        ...
        try {
    		...
    		statementGetMesssageToSend = connection
    				.prepareStatement("select dtl.message_id, dtl.batch_id, dtl.msisdn, dtl.message, mmn.client_sender_id_id, csd.sender_id, "
    						+ "mmn.sending_schedule, dtl.processed_username, cuw.client_id from wa_blast_upload_detail as dtl "
    						+ "left join wa_blast_uploader as mmn on dtl.batch_id = mmn.batch_id "
			...
			statementReadDetail = connection
					.prepareStatement("select vdet.msisdn, vdet.message, vup.client_sender_id_id, csi.sender_id from "
							+ "wa_blast_upload_detail as vdet left join wa_blast_uploader as vup on "
			...
			statementUpdateDetail = connection
					.prepareStatement("update wa_blast_upload_detail set is_sent = true, sent_datetime = ?, "
	...
	public void runTheSendingJob() {
		try {
			System.out.println("Running schedule for sending whatsapp message job.");
			
			JobDetail job1 = JobBuilder.newJob(WABlastSenderNew.class)
	...
	public static void main(String[] args) {
		WABlastSenderScheduler scheduler = new WABlastSenderScheduler();
