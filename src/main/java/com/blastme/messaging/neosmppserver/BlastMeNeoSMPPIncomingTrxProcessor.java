...
class BlastmeNeoSMPPIncomingTrxProcessor implements Runnable {
	...
	public BlastmeNeoSMPPIncomingTrxProcessor(String theMessageId, String theSystemId, String theRemoteIpAddress,
		...
		LoggingPooler.doLog(logger, "DEBUG", "BlastMeNeoSMPPIncomingTrxProcessor", "BlastMeNeoSMPPIncomingTrxProcessor",
				...
				"Session ID: " + sessionId + ". Incoming trx - messageId: " + this.messageId + ", clientSenderId: "
						...
						", dataCoding" + this.dataCoding + ", bShortMessage: " + this.bShortMessage + ", charSet: "
	...
	private Charset getCharsetByDataCoding(byte dataCoding) {
		...
		/**
		 * Octet unspecified (8-bit binary) defined for TDMA and/ or CDMA but not
		 * defined for GSM
		 */
		...
		try {
			switch (dataCoding) {
				case (byte) 0x00:
					theCharSet = CharsetUtil.CHARSET_GSM;
					break;
			...
			LoggingPooler.doLog(logger, "DEBUG", "BlastMeNeoSMPPIncomingTrxProcessor", "getCharsetByDataCoding", false,
					false, false, messageId,
					"sessionId: " + sessionId + " - dataCoding: " + dataCoding + " -> charset: " + theCharSet, null);
		} catch (Exception e) {
			e.printStackTrace();

			LoggingPooler.doLog(logger, "DEBUG", "BlastMeNeoSMPPIncomingTrxProcessor", "getCharsetByDataCoding", true,
					false, false, messageId,
					"sessionId: " + sessionId + " - Failed to get charset of datacoding: " + dataCoding
							+ ". Error occured.",
					e);
	...
	private void saveInitialData(String messageId, LocalDateTime receiverDateTime, String batchId, String receiverData,
		...
		try {
			...
			this.smsTransactionOperationPooler.saveInitialSMPPData(messageId, now, "SMPPAPI", receiverData,
					receiverClientResponse,
					this.remoteIpAddress, now, now, msisdn.trim(), message.trim(), countryCode, prefix, telecomId,
					trxStatus, "SMPP", clientSenderIdId, clientSenderId, this.clientId,
	...
	private void sendRequestPdu(SmppSession session, DeliverSm deliver) {
		try {
			...
			if (!future.await()) {
				LoggingPooler.doLog(logger, "DEBUG", "BlastMeNeoSMPPIncomingTrxProcessor", "sendRequestPdu", false,
				...
				LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "sendRequestPdu", false,
						false, true, messageId,
						"deliver_sm_resp: commandStatus [" + deliverSmResp.getCommandStatus() + "="
			...
			LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "sendRequestPdu", true, false,
					true, messageId,
	...
	private void doProcessTheSMS() {
		...
		try {
			...
			if (isSenderIdValid(this.clientId, this.clientSenderId)) {
				...
				if (jsonMsisdn.getBoolean("isValid")) {
					...
					if (isRouteDefined(this.clientId, this.clientSenderId, this.systemId, telecomId)) {
						...
						if (businessModel.equals("PREPAID")) {
							...
							LoggingPooler.doLog(logger, "DEBUG", "BlastMeNeoSMPPIncomingTrxProcessor",
									...
									"Session ID: " + sessionId + ". Checking initial balance - divisionBalance: "
											+ divisionBalance,
									null);
						} else {
						...
						if (isBalanceEnough) {
							...
							LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "doProcessTheSMS",
									false, false, true, messageId,
									"Session ID: " + sessionId + ". BALANCE ENOUGH, MESSAGE IS ACCEPTED. errCode: "
											+ errorCode + ", esmeErrCode: STATUS OK.",
									null);
							...
							LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "doProcessTheSMS",
									false, false, true, messageId,
									"Session ID: " + sessionId + ". BALANCE NOT ENOUGH. errCode: " + errorCode
						...
						LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "doProcessTheSMS",
								false, false, true, messageId,
								"Session ID: " + sessionId + ". ROUTE IS NOT DEFINED. errCode: " + errorCode
					...
					if (isRouteDefined(this.clientId, this.clientSenderId, this.systemId, telecomId)) {
						...
						LoggingPooler.doLog(logger, "DEBUG", "BlastMeNeoSMPPIncomingTrxProcessor", "doProcessTheSMS",
								false, false, true, messageId,
								"MULTI-COUNTRY CASE - Session ID: " + sessionId + ". Business Mode: " + businessModel,
								null);
						...
						if (isBalanceEnough) {
							...
							LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "doProcessTheSMS",
									false, false, true, messageId,
									"MULTI-COUNTRY CASE - Session ID: " + sessionId + ". BALANCE NOT ENOUGH. errCode: "
											+ errorCode + ", esmeErrCode: SUBMITE FAILED.",
									null);
						}
					} else {
						// Prefix is not valid
						...
						LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "doProcessTheSMS",
								false, false, true, messageId,
								"Session ID: " + sessionId + ". PREFIX IS NOT DEFINED. errCode: " + errorCode
				...
				LoggingPooler.doLog(logger, "INFO", "BlastMeNeoSMPPIncomingTrxProcessor", "doProcessTheSMS", false,
						false, true, messageId,
						"Session ID: " + sessionId + ". SENDERID IS NOT DEFINED. errCode: " + errorCode
								+ ", esmeErrCode: INVALID SOURCE ADDRESS.",
						null);
			}
			...
			// String messageId, LocalDateTime receiverDateTime, String batchId, String
			// receiverData,
			// String receiverClientResponse, String receiverclientIpAddress, LocalDateTime
			// clientResponseDateTime,
			// LocalDateTime trxDateTime, String msisdn, String message, String countryCode,
			// String prefix,
			// String telecomId, String trxStatus, String receiverType, String
			// clientSenderIdId, String clientSenderId,
			// String clientId, String apiUserName, double clientUnitPrice, String currency,
