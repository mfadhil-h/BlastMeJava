...
public class SMSTransactionOperationPooler {
	...
	// CONNECTION, STATEMENT DLL AKAN DIINITIATE PER FUNCTION KARENA SUPAYA THREAD SAFE
	public SMSTransactionOperationPooler() {
		...
		LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "SMSTransactionOperationPooler", false, false, false, "", 
	...
	public void saveInitialSMPPData(String messageId, LocalDateTime receiverDateTime, String batchId, String receiverData, String receiverClientResponse, String receiverclientIpAddress, LocalDateTime clientResponseDateTime, LocalDateTime trxDateTime, String msisdn, String message, String countryCode, String prefix, String telecomId, String trxStatus,  
			String receiverType, String clientSenderIdId, String clientSenderId, String clientId, String apiUserName, double clientUnitPrice, String currency, String messageEncoding, int messageLength, int smsCount){
		...
		try{
			...
			String queryInsert = "INSERT INTO transaction_sms(message_id, transaction_date, msisdn, message, country_code, telecom_id, prefix, status_code, " + 
					...
					"client_price_total, client_sender_id, batch_id, api_username) VALUES ('" + messageId + "', '" + receiverDateTime.format(formatter) + "', '" + msisdn + 
					...
					", " + String.format("%.5f", smsCount * clientUnitPrice) +  ", '" + quote(clientSenderId) + "', '" + batchId + "', '" + apiUserName + "')";
	...
	public void saveTransactionVendor(String messageId, String vendorId, LocalDateTime vendorHitDateTime, String vendorHitRequest,
		...
		try{
			...
			String queryInsert = "INSERT INTO transaction_sms_vendor(message_id, vendor_id, vendor_hit_date_time, vendor_hit_request, "
					...
					+ "VALUES ('" + messageId + "', '" + vendorId + "', '" + vendorHitDateTime.format(formatter) + "', '" + vendorHitRequest 
					+ "', '" + vendorRespDateTime.format(formatter) + "', '" + vendorHitResponse + "', '" + vendorMessageId + "', '" 
			...
			se.printStackTrace();
			LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "saveTransactionVendor", true, false, false, "", 
	...
	public void updateTransactionStatus(String messageId, String transactionStatus){
		...
		try{
			...
			if(transactionStatus.startsWith("00")){
				statementUpdate.execute("update transaction_sms set status_code = '" + transactionStatus.trim() + "' where message_id = '" + messageId + "'");			
			} else {
				// No charge
				statementUpdate.execute("update transaction_sms set status_code = '" + transactionStatus.trim() + "', client_price_per_unit = 0.00, client_price_total = 0.00 where message_id = '" + messageId + "'");
			}
		} catch (Exception e) {
			e.printStackTrace();
			LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "saveInitialSMPPData", true, false, false, "", 
	...
	public String getTransactionStatus(String messageId) {
		...
		try{
			...
			LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "getTransactionStatus", false, false, false, "", 
					"messageId: " + messageId + " -> trxStatus: " + trxStatus, null);
		} catch (Exception e) {
			e.printStackTrace();
			LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "getTransactionStatus", true, false, false, "", 
			...
			try {
				...
				e.printStackTrace();
				LoggingPooler.doLog(logger, "DEBUG", "SMSTransactionOperationPooler", "getTransactionStatus", true, false, false, "", 
						"Failed to close query statement.", e);
			}
		}			
	...
	public JSONObject getTransactionDetail(String messageId) {
		...
		try{
			...
			LoggingPooler.doLog(logger, "DEBUG", "SMSTransactionOperationPooler", "getTransactionDetail", false, false, false, "", 
					"messageId: " + messageId + " -> trxDetail: " + jsonTransaction.toString(), null);
		} catch (Exception e) {
			e.printStackTrace();
			LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "getTransactionStatus", true, false, false, "", 
					"Failed to get trx status. Error occured.", e);
	...
	public void saveTransactionDLR(String messageId, String clientId, LocalDateTime dlrDateTime, String dlrBody, String dlrStatus, String dlrPushTo) {
		Connection connection = null;
		Statement statement = null;
		//Statement statementReceiver = null;
...
//			}
			...
			try {
				...
				e.printStackTrace();
				LoggingPooler.doLog(logger, "DEBUG", "SMSTransactionOperationPooler", "saveTransactionDLR", true, false, false, "", 
						"Failed to close query statement.", e);
			}
		}	
	}
	...
	public void insertTransactionDLRClientResponse(String messageId, String dlrClientResponse) {
		...
		try{
			...
			String queryUpdate = "insert into transaction_sms_dlr_client_resp(message_id, dlr_client_push_response) values ('" + messageId + "', '" + dlrClientResponse + "')";
	...
	public void updateTransactionBatchStatus(String batchId, String trxStatus) {
		...
		try{
			...
			String queryUpdate = "update transaction_sms set status_code = '" + trxStatus + "' where batch_id = '" + batchId + "' and status_code like '004'";
			...
			se.printStackTrace();
			LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "updateTransactionBatchStatus", true, false, false, "", 
	...
	public void deleteBatchIdFromTransaction(String batchId) {
		...
		try{
			...
			String queryUpdate = "delete from transaction_sms where batch_id = '" + batchId + "' and status_code like '004'";
			LoggingPooler.doLog(logger, "INFO", "SMSTransactionOperationPooler", "deleteBatchIdFromTransaction", false, false, false, "", 
	...
	public void saveSMPPBindAttempt(String bindId, LocalDateTime trxDateTime, String apiUserName, String remoteIpAddress, String attemptActivity, String attemptResponse) {
		Connection connection = null;
		Statement statement = null;
		//Statement statementReceiver = null;
		...
		try{
			...
			String queryInsert = "INSERT INTO public.smpp_bind_attempt(bind_id, api_username, remote_ip_address, attempt_activity, attempt_response, trx_datetime) "
					+ "VALUES ('" + bindId + "', '" + apiUserName + "', '" + remoteIpAddress + "', '" + attemptActivity + "', '" + attemptResponse + "', '" + trxDateTime.format(formatter) + "')";
			...
			try {
				...
				e.printStackTrace();
				LoggingPooler.doLog(logger, "DEBUG", "SMSTransactionOperationPooler", "saveSMPPAttempt", true, false, false, "", 
						"Failed to close query statement.", e);
			}
		}	
	}
